## This code investigates the sensitivity to the prognostic score model misspecification. 
## This code can be used to reproduce Tables S2 and S3 in the Supporting Information.
library(MASS)
library(survival)
library(xtable)
source("Code/aux.R")
...
for(ii in 1:500){
  ...
  for(j in 1:J){
    ...
    W1 = runif(nJ, 0, 1)
    #### (M1) misspecified Cox model ####
    T0[j,] = (-log(W1) / (exp(betas[1]*X1[j,] + betas[2]*exp(X2[j,]) + 
                                betas[3]*(X3[j,])^2 + betas[4]*(X4[j,])^3 + betas[5]*X5[j,])*(phi[j])^(tau)))^(1 / tau)
    
    #### (M2) nonlinearity ####
    #T0[j,] = (-log(W1) / (exp(betas[1]*X1[j,]*exp(X2[j,]) + 
    #                            betas[2]*(X3[j,])^(exp(X4[j,])) + betas[3]*X4[j,]*X5[j,])*(phi[j])^(tau)))^(1 / tau)
...
for(ii in 1:M){
  ...
  for(j in 1:J){
    W1 = runif(N, 0, 1)
    #### (M1) misspecified Cox model ####
    T0 = (-log(W1) / (exp(betas[1]*X1 + betas[2]*exp(X2) + 
                            betas[3]*(X3)^2 + betas[4]*(X4)^3 + betas[5]*X5)*(phi[j])^(tau)))^(1 / tau)
    #### (M2) nonlinearity ####
    #T0 = (-log(W1) / (exp(betas[1]*X1*exp(X2) + 
    #                        betas[2]*(X3)^(exp(X4)) + betas[3]*X4*X5)*(phi[j])^(tau)))^(1 / tau)
